    ...
    const completeRow = () => {
        if (currentRow.every((tile) => tile.letter)) {
            ...
            if (!allWords.includes(guess) && guess !== answer) {
                shake()
                showMessage({ msg: `Not in word list` })
                return
            }
            ...
            currentRow.forEach((tile, i) => {
                if (answerLetters[i] === tile.letter) {
                    tile.state = letterStates[tile.letter] = letterStates.CORRECT
            ...
            currentRow.forEach((tile) => {
                if (!tile.state && answerLetters.includes(tile.letter)) {
                    tile.state = letterStates.PRESENT
                    answerLetters[answerLetters.indexOf(tile.letter)] = null
                    if (!letterStates[tile.letter]) {
                        letterStates[tile.letter] = letterStates.PRESENT
                    }
                }
            })
            ...
            currentRow.forEach((tile) => {
                if (!tile.state) {
                    tile.state = letterStates.ABSENT
                    if (!letterStates[tile.letter]) {
                        letterStates[tile.letter] = letterStates.ABSENT
                    }
                }
            })
            ...
            if (currentRow.every((tile) => tile.state === letterStates.CORRECT)) {
                ...
                // game over :(
                setTimeout(() => {
                    showMessage({ msg: answer.toUpperCase(), time: -1 })
                }, 1600)
            }
        } else {
            shake()
            showMessage({ msg: 'Not enough letters' })
        }
    }
    const genResultGrid = () => {
